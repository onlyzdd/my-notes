...
```bash
$ chown test a.txt # 改变 a.txt 的所有者为用户 test
$ chown .staff a.txt # 改变 a.txt 的所有组为 staff
```
...
`whereis` 命令是定位可执行文件在文件系统中的位置，类似于 `where` 命令。
...
`which` 命令用来查看可执行文件的位置，并返回第一个搜索结果，可执行文件的位置包括 `PATH` 环境变量，Shell 内置命令和自定义的函数、别名等。
